...
- If selfie video → transcribe via `transcribe_audio`, note vocabulary, energy, pacing, fillers, what they're enthusiastic about.
...
- **Who's already winning here at your size** — name 2–3 creators at 5K–30K who run this exact play, with one sentence each on what makes them work. Only name real creators, handles, follower tiers, or current metrics if verified during this run via `scrape_social` or another live supported source. If you cannot verify, use clearly labeled archetypes instead (e.g. "local style micro-creator archetype") and omit handles/counts.
...
1a. **Handle-only public-feed fallback for video-heavy / mostly Reels accounts.** If the user gave a public handle but no separate photo files, scraped feed imagery can provide curated tiles. For image posts, use the highest-quality `image_versions2.candidates[0].url`. For video posts where `media_url` is a `.mp4`, do not use the ephemeral IG `visual_media_url` / signed cover JPG as the curated tile source; do not alter any signed-cover query param because that causes `URL signature mismatch`, and mid-clip covers often include baked-in caption or title overlays. Use only the durable rehosted `.mp4` returned by the Step 1 `rehost: true` scrape (the `pika_cdn_url` / `cdn.pika.art` media; treat it as `is_durable: true`) and call `extract_frame(video_url: <durable_rehosted_mp4>, time_s: 0)`. Treat each extracted frame as a candidate tile, not an approved tile: immediately call `analyze_media` on the frame URL with an OCR / visible text / no-text prompt before placement. This sourcing-time check must happen before the frame becomes a mood-board tile, PDF content-category tile, or voice-page curated tile. If frame 0 contains text, a title card, or a burned-in caption, drop the reel or try a later keyframe (for example `time_s: 1.5` or `time_s: 3`) and run the same `analyze_media` no-text check before using that later keyframe. If no durable rehosted `.mp4` is present, retry the scrape with `rehost: true`; do not fall back to a signed cover URL. Final PDF QA is only a backstop after this sourcing gate, never the first text-detection step.
...
6. **Fill gaps with `generate_image` (provider="gpt-image-2")** for **social-content atmosphere only** *(load-bearing phrase — pushes gpt-image-2 away from vision-board / wallpaper energy; see Load-bearing phrases section)*, prompted in the Step 2 aesthetic (its palette, lighting, settings, photography style). See the hard rules below. Generated tiles go in `mood-board/generated/`.
...
Then call `html_to_png` with `format: "png"` and `raster_options.viewport_px = { width: 1920, height: 1224 }`. If it returns `{task_id, status}`, poll `task_status({task_id})` until terminal and read the image URL. **For the PDF full-bleed page (Step 4 page 3)**, render the SAME tiles through `references/templates/mood-board-no-header.template.html` at `{ width: 1920, height: 1080 }` — that variant has zero gutters so the PDF page shows no cream slivers.
...
- **Page count:** exactly 12 pages maximum, each focused on one idea. Do not expand the kit past 12 pages: the required `analyze_media` PDF QA path uses `all_pages: true`, and that sync path supports at most 12 PDF pages.
...
- **Split: 2 curated + 2 freshly-generated tiles per page.** Curated = scraped feed images at higher quality than what's in the mood board (different posts). For handle-only video-heavy / mostly Reels accounts, treat clean `extract_frame(video_url: <durable_rehosted_mp4>, time_s: 0)` outputs from `.mp4` `media_url` posts as candidate curated feed images; do not use altered `visual_media_url` signed covers. Before any reel-derived frame is placed on a voice page or content-category card, call `analyze_media` on the candidate frame with an OCR / visible text / no-text prompt. Reject frames with baked-in caption/title overlays; if you try a later keyframe, run the same `analyze_media` no-text check on that later keyframe before placement. If there is no feed/no social yet, use different user-provided camera-roll photos instead. If there are no curated user photos or clean public-feed curated frames, do not build the PDF; pause and request real photos.
...
- Reference images as HTTPS URLs — for local images, call `upload_asset` first to get a CDN URL, then reference that URL in the HTML.
- **🛑 LOAD-BEARING: pass `pdf_options.paper_size` explicitly to `html_to_pdf`.** The `@page { size: 1920px 1080px }` CSS rule alone is **not enough** — `html_to_pdf` defaults to US Letter (792×612 points, 1.294 aspect ratio) if `pdf_options.paper_size` is omitted, and the 1920×1080-designed HTML gets scaled/letterboxed into the letter page with giant white bars top and bottom. Always pass:
  ```json
  "pdf_options": {
  ...
  ```
  `html_to_pdf` defaults to async mode. If the response is `{task_id, status}`, poll `task_status({task_id})` in a tight loop until `completed`, `failed`, or `cancelled`; on `completed`, read the PDF URL from the task result before proceeding. Keep the completed `html_to_pdf` / `task_status` result in notes; its server-side structural metadata (`page_count`, and `pages[]` when present) is the only portable page-count evidence for the timeout fallback. Verify after each render by calling `analyze_media` on the rendered PDF as `application/pdf` with `all_pages: true`. Ask it to inspect all pages for 16:9 landscape framing, no US Letter letterboxing, no giant white bars, no blank trailing pages, and no page that breaks the visual hierarchy. If `all_pages: true` times out on an image-dense PDF, use the Step 4e per-page QA + server-result page-count / `pdf_options.paper_size` fallback before deciding whether the render can proceed. If it reports letterboxing or white bars, rerender with the explicit `pdf_options.paper_size` above.
- Save the resulting PDF as `./tmp/[handle]-influencer-kit/influencer-persona.pdf` (never `media-kit.pdf`).

**🛑 PDF size cap: html_to_pdf rejects outputs over 50 MB.** gpt-image-2 PNG outputs are 1–3 MB each; with ~15–20 images per kit, raw PNG embeds can push past the cap on the first render. **Before final PDF render, reduce large generated images through MCP, not local image libraries:** wrap each large image URL in a one-image HTML/CSS frame and call `html_to_png` with `format: "jpg"`, `raster_options.viewport_px` capped so the longest side is ≤1400, and `raster_options.jpeg_quality: 85`. If the response is `{task_id, status}`, poll `task_status({task_id})` until terminal and use the completed JPG URL in the PDF HTML.

**Canonical generated-tile URL pipeline (MCP-only):**
1. Generate each atmosphere tile with `generate_image` (provider="gpt-image-2").
2. For every generated PNG that will be embedded in the PDF, transcode/downscale through `html_to_png` by placing the source URL as a full-bleed background image in a small HTML frame and rendering `format: "jpg"` at quality 85.
...
5. Call `html_to_pdf` with `body_pages`, `shared_head`, and the explicit `pdf_options.paper_size` above.
...
**🛑 Cloudflare R2 / `upload_asset` PUT gotchas:**
...
1. **Render-read every page through MCP.** Call `analyze_media` on the final PDF URL as `application/pdf` with `all_pages: true` and ask for a page-by-page QA pass: confirm every page renders cleanly in 16:9 landscape, no blank pages, no US Letter white-bar letterboxing, no confusing/off-aesthetic pages, no broken hierarchy, and no visible image/crop failure. If any issue is reported, fix the HTML/assets and rerender before delivering.
   - **Fallback only for image-dense PDF timeout / rasterization budget failures:** `all_pages: true` is still the mandatory first QA call. If that all-pages call times out or fails only because the multi-page PDF rasterization budget was exhausted, do not block the run solely on that aggregate failure. Verify structure from MCP/server evidence first: the completed `html_to_pdf` or `task_status` result must report `page_count: 12` (and `pages[]` length 12 when present), the render request must have exactly 12 `body_pages`, and the submitted `pdf_options.paper_size` must be `{width: 1920, height: 1080, unit: "px"}` with zero margins. Treat that server-render result + request as the MediaBox/page-size contract; do not run local PDF inspection or local PDF tools/libraries. If the server result lacks `page_count`, the fallback is not proven: rerender with smaller JPG assets or stop and report `pdf_structural_metadata_unavailable`. Once structure is proven, run per-page MCP QA with `analyze_media(media=<pdf_url>, application/pdf, page: <n>)` for page 1 through page 12, asking the same visual QA question for each page. Any per-page visual defect still blocks delivery and requires rerender.
...
| Step 3 (mood board: font pick + curate + GPT fill + composite) | 6–10 min | gpt-image-2 generations + HTML/CSS composite rendered through `html_to_png` |
...
- **Render generated PNGs through `html_to_png` before PDF embedding** — the MCP JPG conversion keeps the final PDF under the 50 MB cap and lets CSS filters/overlays match the generated tile to nearby real-photo register. This is size + presentation hygiene; it cannot rescue bad generated composition.
...
| `scrape_social` returns "not found" | Handle typo or wrong platform (gave you a TikTok handle thinking it was IG) | Confirm the handle with the user, retry with the correction. |
| Taste URL sent to `scrape_social` and fails | Spotify / Depop / Letterboxd / personal sites are taste signals, not supported `scrape_social` platforms | Do not call `scrape_social` for unsupported taste URLs. Record what the URL suggests; if unclear, ask what dimension to borrow from it. |
| `scrape_social` returns "private account" | Account is private | Ask the user to set it public temporarily OR paste 5–10 recent captions in chat. If they cannot, mark the voice bank as inferred from chat answers and keep caption examples conservative. |
| `scrape_social` throttled | Platform rate-limited the worker | Wait 60s and retry once. If still throttled, fall back to manual caption paste. |
| `scrape_social` returns posts but no caption text | Photos-only / reels-only account with no caption bodies | Use bio + chat-voice as voice signal. Note in `persona.md` that captions weren't available so other skills know the voice was inferred. |
| `URL signature mismatch` when trying to use a reel cover | Used or modified the ephemeral IG `visual_media_url` / signed cover URL instead of a durable video asset; signed cover query params must stay untouched and are not reliable curated-tile sources | For handle-only Reels accounts, ignore the signed cover for curated tiles. Use the durable rehosted `.mp4` `media_url` (for example `cdn.pika.art`) and call `extract_frame(video_url: <durable_rehosted_mp4>, time_s: 0)` to create the tile. |
| Reel-derived curated tile has baked-in caption/title overlays | Treated an extracted reel frame as already approved, skipped the sourcing-time `analyze_media` OCR/no-text check, or sampled a mid-clip reel frame after the creator's text appeared | Extract frame 0 / `time_s: 0` from the durable rehosted `.mp4`, then call `analyze_media` on that candidate frame before placement. If frame 0 contains visible text, a title card, or a burned-in caption, drop the reel or try a later keyframe and run the same `analyze_media` no-text check before using it. Mood-board and voice-page tiles must have no text on tiles; final PDF QA is only a backstop. |
...
| Mood board has visible misaligned / overlapping tiles | HTML/CSS grid dimensions or `object-position` values were off | Re-check tile dimensions and grid spacing. Re-render with `html_to_png`. Don't ship a misaligned board. |
...
| PDF has giant white bars at the top and bottom of every page | Forgot to pass `pdf_options.paper_size` to `html_to_pdf` — the @page CSS rule is not enough; the tool defaults to US Letter (792×612, 1.29 aspect) and the 1920×1080 HTML letterboxes into it | Always pass `pdf_options: { paper_size: {width: 1920, height: 1080, unit: "px"}, margins: {top:0, right:0, bottom:0, left:0, unit:"px"} }`. After render, call `analyze_media` on the PDF as `application/pdf` with `all_pages: true` and ask it to flag white-bar letterboxing or any non-16:9 page. |
| `analyze_media(all_pages: true)` times out on an image-dense final PDF | Multi-page PDF rasterization budget exhausted before Gemini sees every page | Use the Step 4e fallback only with portable evidence: `html_to_pdf` / `task_status` result `page_count: 12` (and `pages[]` length 12 when present), original `body_pages.length === 12`, and explicit 1920×1080 `pdf_options.paper_size`. Then run `analyze_media` with `page: <n>` for page 1 through page 12. If `page_count` is missing, rerender smaller or stop with `pdf_structural_metadata_unavailable`; do not use local PDF tools/libraries. |
...
| html_to_pdf rejects image asset with "MIME type 'text/html' is not in the allowlist" | A parallel subshell `curl PUT` after `upload_asset` dropped the `Content-Type: image/jpeg` header silently; R2 stored the JPG with `text/html` MIME | After every batch PUT, verify with `curl -s -I <url> \| grep content-type` and re-PUT any with wrong types. Or use sequential PUTs (but watch the 5-min URL TTL — see next row). |
