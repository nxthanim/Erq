...
// Clerk removed; using legacy JWT + Supabase OAuth flows
...
export default function Login() {
  ...
  // Clerk disabled — render legacy login form below

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}
      className="min-h-screen flex flex-col md:flex-row" style={{ backgroundColor: '#ffffff' }}>
      <SeoHead
        title="Sign in | Erq Marketplace"
        description="Sign in to your Erq Marketplace account to post jobs, manage gigs, and message clients or freelancers."
        path="/login"
        noindex
      />
        ...
        <div className="relative text-center max-w-md">
          ...
          <div className="relative hidden sm:block h-[120px] mb-8">
            <div className="absolute left-0 top-0 flex items-center gap-2 px-3 py-2 rounded-full" style={{ backgroundColor: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', animation: 'float 6s ease-in-out infinite' }}>
              ...
              <span className="text-xs" style={{ color: '#ffffff', fontFamily: 'var(--font-sohne)' }}>Direct payments between parties</span>
            </div>
            ...
            <div className="absolute left-6 bottom-0 flex items-center gap-2 px-3 py-2 rounded-full" style={{ backgroundColor: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', animation: 'float 6s ease-in-out 4s infinite' }}>
              ...
              <span className="text-xs" style={{ color: '#ffffff', fontFamily: 'var(--font-sohne)' }}>Smart matching</span>
            </div>
          </div>
      ...
      >
        <div className="w-full max-w-md">
          ...
          {/* Kinde hosted authentication */}
          <div className="mt-6 text-center">
            <button
              onClick={() => login().catch(err => setError(err?.message || 'Kinde sign-in failed'))}
              className="w-full inline-flex items-center justify-center gap-3 px-4 py-2 rounded-full border"
              style={{ borderColor: '#ececec', backgroundColor: '#ffffff', color: '#173a32' }}
            >
